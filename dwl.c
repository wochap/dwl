...
enum { LyrBg, LyrBottom, LyrTile, LyrTop, LyrFloat, LyrOverlay, LyrBlock, NUM_LAYERS }; /* scene layers */
int LyrFS = LyrFloat;
#ifdef XWAYLAND
...
switchxkbrule(const Arg *arg)
{
	...
	struct xkb_keymap *keymap;
	struct xkb_rule_names newrule;

	xkb_rule_index = (xkb_rule_index + 1) % LENGTH(xkb_rules);
	newrule = xkb_rules[xkb_rule_index];
...
tile(Monitor *m)
{
	...
	wl_list_for_each(c, &clients, link) {
		if (!VISIBLEON(c, m) || c->isfloating || c->isfullscreen)
			continue;
		if (i < m->nmaster) {
			h = ROUND((m->w.height - my) * (c->cweight / mweight));
			resize(c, (struct wlr_box){.x = m->w.x, .y = m->w.y + my,
				.width = mw, .height = h}, 0);
			my += c->geom.height;
			mweight -= c->cweight;
		} else {
			h = ROUND((m->w.height - ty) * (c->cweight / tweight));
...
bstack(Monitor *m)
{
	...
	if (n > m->nmaster)
		mh = ROUND(m->nmaster ? m->w.height * m->mfact : 0);
	else
		mh = m->w.height;
	i = mx = tx = 0;
	wl_list_for_each(c, &clients, link) {
